...
export const ReactFlowPage3: React.FC<ReactFlowPage3Props> = ({ onInit }) => {
  ...
  const buildGraph = useCallback((deps: typeof departments) => {
    ...
    deps.forEach((dept, deptIndex) => {
      ...
      // TL X 계산 (Quality 부서는 특별 처리)
      const tlXs = isQuality 
        ? catXs.filter((_, idx) => isCatChildOfTL(idx)) // TL 직하단 TM 카테고리의 X 좌표 사용
        : computeTLXs(tlCount, deptCenterX, catXs, LAYOUT_CONFIG.COL_SPACING);

      // GL X 계산: 직하단 TL 평균
      const glXs = computeGLXs(glCols, deptCenterX, tlXs);
      ...
      if (glCols > 0) {
        for (let i = 0; i < glCols; i++) {
          ...
          glIds.push(glId);
          const glX = glXs[i] ?? (deptCenterX + (i - (glCols - 1) / 2) * LAYOUT_CONFIG.COL_SPACING);
          nodes.push({
            id: glId,
            type: 'customPosition',
      ...
      dept.tl.forEach((tl: string, tlIndex: number) => {
        ...
        tlIds.push(tlId);
        const tlX = tlXs[tlIndex] ?? (deptCenterX + (tlIndex - (Math.max(dept.tl.length, numColumns) - 1) / 2) * LAYOUT_CONFIG.COL_SPACING);
        nodes.push({ id: tlId, type: 'customPosition', position: { x: tlX - 70, y: getHierarchyY('TL') }, data: { title: 'TL', subtitle: tl, level: 3, colorCategory: isTPM ? 'OH' : getColorCategory(dept.title, 'TL', tl) } });
        if (glIds.length > 0) {
      ...
      dept.tm.forEach((tmCat: string[], catIdx: number) => {
        ...
        tmCat.forEach((tm, tmIdx) => {
          ...
          nodes.push({ id: tmId, type: 'customPosition', position: { x: tmX - 70, y: tmY }, data: { title: 'TM', subtitle: tm, level: 4, colorCategory: isTPM ? 'OH' : getColorCategory(dept.title, 'TM', tm) } });
          if (tmIdx === 0) {
            ...
            // TM → TM: 같은 레벨 내 연결
            edges.push({ id: `${prev}-${tmId}`, source: prev, target: tmId, type: 'smoothstep', pathOptions: { offset: LAYOUT_CONFIG.EDGE_OFFSET } });
          }
          prev = tmId;
        });
      });
