...
export const ReactFlowPage1: React.FC<ReactFlowPage1Props> = ({
  ...
  const createNodesAndEdges = React.useCallback(() => {
    ...
    // 1) 공통 LAYOUT_CONFIG 사용으로 통일
    const levelHeight = LAYOUT_CONFIG.LEVEL_HEIGHT;
    const glSpacing = LAYOUT_CONFIG.COL_SPACING; // 160px - 공통 열 간격 사용
    const linePadding = LAYOUT_CONFIG.LINE_PADDING; // 라인간 간격 (공통 설정 사용)
    const standardBendDistance = LAYOUT_CONFIG.STANDARD_BEND_DISTANCE; // 표준 꺾임 거리

    // ===== 1. 각 라인의 너비를 미리 계산 =====
    const lineEntities = Array(config.lineCount).fill(null).map((_, lineIndex) => {
      ...
      const processCount = mainProcesses.length;
      return { width: calculateLineWidth(processCount) };
    });

    // ===== 2. 공통 배치 함수를 사용하여 라인 위치 계산 =====
    const { positions: linePositions, totalWidth, separatedStartX } = calculateEntityPositions(
      lineEntities,
      LAYOUT_CONFIG.DEPT_PADDING // 분리된 공정 간격
    );
    ...
    for (let vsmIndex = 0; vsmIndex < vsmCount; vsmIndex++) {
      ...
      // LM 위치 계산 (관리하는 라인들의 중앙) - 공통 함수의 결과 사용
      const vsmX = managedLines.reduce((sum: number, line: any) => {
        return sum + linePositions[line.lineIndex];
      ...
      // PM → LM: 표준 꺾임 거리 사용
      const pmBottomY = getHierarchyY('VSM') + 64; // PM 박스 하단
      edges.push({ id: `edge-${pmId}-${vsmId}`, source: pmId, target: vsmId, type: 'customCenterY', data: { centerY: pmBottomY + standardBendDistance } });
    }
    ...
    Array(config.lineCount).fill(null).forEach((_, lineIndex) => {
      // 공통 함수로 계산된 라인 중심 X 좌표 사용
      const lineCenterX = linePositions[lineIndex];
      const lineWidth = lineEntities[lineIndex].width;
      ...
      const glStartX = lineStartX + (linePadding / 2);
        ...
        processGroup.tlGroup.forEach((tl: any, tlIndex: number) => {
          ...
          if (tlIndex === 0) {
            ...
            if (processGroup.showGL !== false && glId) {
              ...
              // LM → TL (skip): 표준 꺾임 거리 사용
              const lmBottomY = getHierarchyY('A_VSM') + 64; // LM 박스 하단
              edges.push({
                id: `edge-${sourceId}-${tlId}`,
                source: sourceId,
                target: tlId,
                type: 'customCenterY',
                data: { centerY: lmBottomY + standardBendDistance }
              });
            }
          } else {
            edges.push({
    ...
    // ===== 6. 분리된 공정 노드 생성 =====
    // 공통 함수에서 계산된 separatedStartX 사용
    ...
    let currentSeparatedX = separatedStartX!; // 공통 함수에서 계산된 값 사용
    ...
    if (linesWithNosew.length > 0) {
      ...
      colXs.forEach((xPos, idx) => {
        if (idx < requiredGLCount) {
          ...
          // PM → GL: 표준 꺾임 거리 사용
          const pmBottomY = getHierarchyY('VSM') + 64;
          edges.push({ id: `edge-${topConnectionNodeId}-${glId}`, source: topConnectionNodeId, target: glId, type: 'customCenterY', data: { centerY: pmBottomY + standardBendDistance } });
        }
      });
        ...
        for (let shiftIdx = 0; shiftIdx < shiftCols; shiftIdx++) {
          ...
          if (!columnTLHistory[colIndex] || columnTLHistory[colIndex].length === 0) {
            ...
            if (sourceId === topConnectionNodeId) {
              // PM → TL: 표준 꺾임 거리 사용
              const pmBottomY = getHierarchyY('VSM') + 64;
              edges.push({ id: `edge-${sourceId}-${tlId}`, source: sourceId, target: tlId, type: 'customCenterY', data: { centerY: pmBottomY + standardBendDistance } });
            } else {
    ...
    if (linesWithHfWelding.length > 0) {
      currentSeparatedX += LAYOUT_CONFIG.DEPT_GUTTER; // Page2와 동일한 부서간 간격 사용 (100px)
      ...
      linesWithHfWelding.forEach((lineIndex, i) => {
        ...
        hfXs.forEach((xPos, colIdx) => {
          ...
          nodes.push({
            ...
            if (i === 0) {
            // PM → TL 첫 연결: 표준 꺾임 거리 사용
            const pmBottomY = getHierarchyY('VSM') + 64;
            edges.push({ id: `edge-${topConnectionNodeId}-${tlId}`, source: topConnectionNodeId, target: tlId, type: 'customCenterY', data: { centerY: pmBottomY + standardBendDistance } });
