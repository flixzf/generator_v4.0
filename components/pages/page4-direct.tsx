              ...
              return (
                <React.Fragment key={`line-${lineIndex}`}>
                  {model.processes.map((process, processIndex) => (
                    <tr key={`${model.styleNo}-${process.name}-${lineIndex}`} className={lineIndex % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                      {processIndex === 0 && (
                        <>
                          <td className="border p-1" rowSpan={model.processes.length}>{lineIndex + 1}</td>
                          <td className="border p-1" rowSpan={model.processes.length}>{model.category}</td>
                          <td className="border p-1" rowSpan={model.processes.length}>{model.modelName}</td>
                          <td className="border p-1" rowSpan={model.processes.length}>{model.styleNo}</td>
                        </>
                      )}
                  ...
                  <tr className="bg-gray-200 font-bold">
                    <td colSpan={5} className="border p-1 text-right">SUB TOTAL (Line {lineIndex + 1})</td>
